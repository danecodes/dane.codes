...
Let me describe a scene that every Roku developer on earth has lived through.
...
Here's what separates Uncle Jesse from "I wrote a wrapper around curl." `LiveElement` is a persistent reference to a UI element that re-queries the device every time you call a method on it. It doesn't cache stale state. It doesn't hold a reference that goes dead when the screen changes. It goes back to the device and checks.
...
This is my favorite thing in the whole framework and I don't care if that sounds earnest.
